...
defineFeature(feature, test => {
  ...
  test("User should be able to click an event button to see details", ({
    given,
    when,
    then
  }) => {
    ...
    then("the user should see more information on that event", () => {
      expect(AppWrapper.find(".event__Overview")).toHaveLength(1);
    });
  });

  test("User should be able to click an event button to see minimize details", ({
    given,
    and,
    when,
    then
